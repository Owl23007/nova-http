---
description: 通用流式响应的原始需求与验收目标，保留历史决策上下文。
status: 历史需求
---
...
::: info 阅读范围
流式响应已在 0.2.0 引入，0.2.1 调整了超时与关闭行为。本文保留原始需求和验收目标，不代表所有目标已经通过验证。当前用法见[流式响应](../guide/streaming-response)，实现边界见[响应与取消](../framework/internals/response-lifecycle)。

文中的“当前”指提案编写时：工作区后来已加入流式请求体，旧 HttpParser 与连接层结构也已调整。
:::
